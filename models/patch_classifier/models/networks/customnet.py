...
def make_patch_xceptionnet(layername="block2", num_classes=2):
    def change_out(layers):
        try:
            ind, layer = [(i, l) for i, (n, l) in enumerate(layers) if n == layername][
                0
            ]
        except IndexError:
            print("layer %s not found" % layername)
            exit(0)
    ...
    pretrained_dict = torch.load(
        os.path.join(os.getcwd(), "pretrain", "xception-43020ad28.pth")
    )
    ...
    print(f"Total layers in model: {len(model_dict)}")
    print(f"Successfully matched and loaded: {len(pretrained_dict)} layers.")
    ...
    def __init__(
        ...
        sequence = modify_sequence(
            [
                ...
                (
                    "conv3",
                    xception.SeparableConv2d(channels[14], channels[15], 3, 1, 1),
                ),
                ("bn3", nn.BatchNorm2d(channels[15])),
                ("relu3", nn.ReLU(inplace=True)),
                (
                    "conv4",
                    xception.SeparableConv2d(channels[15], channels[16], 3, 1, 1),
                ),
