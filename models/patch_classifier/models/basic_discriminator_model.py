...
from models.patch_classifier.utils import renormalize, imutil
...
class BasicDiscriminatorModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_D = networks.define_D(
            opt.which_model_netD, opt.init_type, self.gpu_ids
        )
    ...
    def compute_losses_D(self):
        ...
        self.acc_D = torch.mean(
            torch.eq(self.labels, torch.argmax(self.pred_logit, dim=1)).float()
        )
